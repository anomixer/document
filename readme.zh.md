...
- 📦 **PWA 支持** — 可安装，支持离线使用
- 🌍 **多语言** — 支持英文、简体中文、繁体中文及智能自动识别
- 🧩 **可嵌入** — 完整的 postMessage API 支持 iframe 集成

---

## ⚡ 与上游版本对比优势 (`anomixer/document` vs Upstream)

| 功能 / 修复项目        | 上游原始版本 (`chaxus/document`)                     | 本 Fork 版本 (`anomixer/document`)                                     |
| :--------------------- | :--------------------------------------------------- | :--------------------------------------------------------------------- |
| **语系自动识别**       | ⚠️ 偏好切换易失效（选择英文语系依然显示简中菜单）    | 🌐 **智能自动识别**：无缝切换 `EN` / `zh-CN` / `zh-TW` 并保留 URL 参数 |
| **Word 英文模式**      | ❌ 点击 File 菜单崩溃白屏 (`TypeError: textContent`) | 🛠️ **完整修复**：补齐缺失的英文翻译 Key                                |
| **繁体中文 (`zh-TW`)** | ❌ 缺失 / 不支持台湾地区设置                         | 🇹🇼 **完整支持**：OpenCC `s2twp` 自动转译与 `LCID 1028` (NT$)           |
| **桌面版应用**         | ❌ 仅支持网页版                                      | 🖥️ **Tauri 桌面端**：支持 Windows 原生桌面可执行文件 (`.exe`)          |
