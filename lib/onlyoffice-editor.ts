...
// Dark Mode: resolve the effective theme (explicit override -> stored pref ->
// system), then apply it to the OnlyOffice editor (customization.uiTheme) and
// the surrounding app (content-theme). v9 does not ship this; grafted from v7.
...
function createPersonalEditorInstance(config: {
  ...
  // Reflect the current theme into the app-level prefs so the surrounding UI
  // (landing/toolbar) matches the editor we are about to mount.
  try {
    const dark = isDarkMode();
    localStorage.setItem('ui-theme-id', dark ? 'theme-dark' : 'theme-classic-light');
    localStorage.setItem('content-theme', dark ? 'dark' : 'light');
  } catch {
    // Ignore storage errors in restricted contexts
  }
  ...
  window.editor = new window.DocsAPI.DocEditor('iframe', {
    ...
    editorConfig: {
      ...
      customization: {
        // Dark Mode: apply the effective theme to the editor UI (v9 theme
        // catalog uses "theme-dark" / "theme-classic-light").
